...
export async function GET(req: Request) {
  ...
  const qpHost = url.searchParams.get('host') || undefined;
  ...
  const originalConnectionString = process.env.DRIZZLE_DATABASE_URL || process.env.DATABASE_URL;
  ...
  // DNS-over-HTTPS resolver (A -> CNAME->A)
  ...
  // Test 1: Direct pool (strict when CA present)
  try {
    const pool1 = new Pool({ connectionString, ssl: sslConfig });
    const result = await pool1.query('SELECT 1 as test');
    await pool1.end();
    results.tests.push({ name: 'Connection (with CA if available)', success: true, result: result.rows[0], hasCert: !!caArray, servername: hostname, connectionString });
  } catch (error: any) {
    results.tests.push({ name: 'Connection (with CA if available)', success: false, error: error.message, hasCert: !!caArray, servername: hostname, connectionString });
  }
